---
layout: post
title: Definitions of Domain-driven Design
tags: [random]
---
...
It is thus very important that the domain model is not dependent on any infrastructural concerns.  The domain model needs to be ignorant of how data is stored, where systems are hosted, and anything else which is not applicable to the domain problem and that which would influence the design of such a model.  Invariants become lost and eventually unacknowledged as infrastructural code begins to dominate.
...
*An entity is a real thing in our domain which is not defined by its properties but rather by its identity.*
...
*An aggregate is a cluster of associated objects that we treat as a unit for the purpose of data consistency, integrity and invariant preservation.*
...
*A repository mediates between the domain and data mapping layers using a collection-like interface for retrieving, adding and updating aggregates.*
...
*A domain service is used when a process or transformation in the domain is not a natural responsibility of an entity or value object.*
...
*Domain events are used to capture an occurrence of something that happened in the domain.*
